...
async function warmUpBackend(force = false) {
...
async function apiJson(url, options = {}) {
    ...
    const retryDelayMs = Number(options.retryDelayMs || 700);
    ...
    for (let attempt = 0; attempt <= retries; attempt += 1) {
        ...
        try {
            ...
            if (error?.name === "TypeError") {
                ...
                const originHint = isFileProtocol ? "http://127.0.0.1:5500" : window.location.origin;
                const networkError = new Error(
                    `Network/CORS error. Could not reach API at ${apiTarget} from origin ${originInfo}. ` +
                    `If backend is deployed, add this origin to ALLOWED_ORIGINS: ${originHint}`
                );
...
function updateChatStatus(mode) {
    ...
    const safeMode = ["ready", "thinking", "error"].includes(mode) ? mode : "ready";
...
async function fetchChatReply(userMessage, context) {
    await warmUpBackend();
